...
final class SyncSquashingTests: XCTestCase {
    func test_syncSquash_putPlusPut_mergesPayloads() async throws {
        ...
        XCTAssertEqual(pending.first?.operation, .put)
        ...
        XCTAssertEqual(merged.expectedVersion, 10)
    }

    func test_syncSquash_postPlusPut_foldsIntoCreate() async throws {
        let container = try makeInMemoryContainer()
        let store = await MainActor.run { LocalSyncOperationStore(container: container) }
        ...
        let pending = try await store.fetchPending(orgId: orgId)
        XCTAssertEqual(pending.count, 1)
        XCTAssertEqual(pending.first?.operation, .post)

        let merged = try JSONCoding.decoder.decode(CreateTaskRequest.self, from: Data((pending[0].payloadJSON ?? "").utf8))
        XCTAssertEqual(merged.id, taskId)
        XCTAssertEqual(merged.title, "New")
    }

    func test_syncSquash_postPlusDelete_becomesNoop() async throws {
        let container = try makeInMemoryContainer()
        let store = await MainActor.run { LocalSyncOperationStore(container: container) }
